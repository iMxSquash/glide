...
11. ~~**Mise en ligne — étape C** : WebRTC côté Electron, en remplacement direct du mode LAN (décision d'Elwen, pas de coexistence des deux transports).~~ ✅ Fait (branche `feat/webrtc-signaling`, pas mergée)
12. ~~**Mise en ligne — étape D** : WebRTC côté PWA (l'app fonctionne à nouveau de bout en bout en local : signaling + PC + téléphone).~~ ✅ Fait (branche `feat/webrtc-signaling`, pas mergée)
13. **Mise en ligne — étapes E→F** : déploiements Render (signaling) + Vercel (PWA), puis validation multi-utilisateurs/latence/WiFi isolé et release `v1.2.0`. ← prochaine étape

> **P0, P1 et la quasi-totalité du P2 sont traités, et les étapes A→D de la mise en ligne aussi.** Le workflow `release.yml` a maintenant vraiment tourné (contrairement à avant) : ça a révélé et corrigé un bug réel (`electron-builder` exige une version Electron fixe, pas un range semver). La release `v1.0.0` est publiée avec le `.exe`, GitHub Pages est désactivé et nettoyé (remplacé par Vercel dans la suite du plan), le logo a été mis à jour partout (icônes + UI), le serveur de signaling WebRTC (`apps/signaling`) est écrit et testé, le serveur Electron parle WebRTC (étape C), et la PWA sait maintenant négocier ce WebRTC de bout en bout (étape D, testé en conditions réelles sans mock, y compris un bug de sync volume trouvé et corrigé au passage). **L'app fonctionne à nouveau de bout en bout en local** (signaling + PC + téléphone sur le même poste) sur la branche `feat/webrtc-signaling`, mais rien n'est encore déployé : c'est l'étape E, à traiter avant tout merge sur `main`. Seul le P2 "tests sur devices réels" et le test manuel de l'installeur `.exe` restent hors de portée sans matériel physique.
