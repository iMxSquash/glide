...
### Étape C — Adapter le serveur Electron (WebRTC côté PC) 🟡 En cours (branche `feat/webrtc-signaling`)
...
- [ ] **Additif pour l'instant, pas remplacement** : le mode LAN direct actuel continue de tourner tel quel en parallèle. Le WebRTC est opt-in via `GLIDE_SIGNALING_URL` (absent = comportement inchangé pour tout le monde). La suppression du serveur HTTPS local/cert/firewall/Express/affichage IP multi-interfaces attendra une validation réelle (après étapes D+E, avec un vrai téléphone).
- [ ] Le QR code et la fenêtre PIN affichent encore l'IP/port LAN, pas `https://<app>.vercel.app/#s=<sessionId>` — pas de PWA WebRTC (étape D) ni d'URL Vercel (étape E) à afficher pour l'instant, un lien qui ne mène nulle part n'aiderait pas. À faire une fois D+E prêtes.
